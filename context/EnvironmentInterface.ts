...
/* ─────────────────────────────────────────────────────────
DOC: Interface de entorno — context/EnvironmentInterface.ts
QUÉ HACE:
  Define tipos/contratos para variables de entorno usadas en cliente/servidor,
  evitando “strings mágicos” y permitiendo autocompletado/seguridad de tipos.
...
API / EXPORTS / RUTA:
  ...
  — export interface ServerEnv {
      ...
      // otros secretos server-only
    }

USO (ejemplo completo):
  // Tipo | opcional | valores permitidos | default
  import type { PublicEnv } from "@/context/EnvironmentInterface";
  const env = process.env as unknown as PublicEnv;

NOTAS CLAVE:
  — Seguridad: NUNCA usar claves server en cliente. Prefijar con NEXT_PUBLIC_ solo lo público.
  — DX: facilita refactors y auditoría de variables usadas.
  — Entornos: diferenciar .env.local (dev) y variables del deploy (Vercel).

DEPENDENCIAS:
  TypeScript tipos globales (NodeJS.ProcessEnv)
────────────────────────────────────────────────────────── */
