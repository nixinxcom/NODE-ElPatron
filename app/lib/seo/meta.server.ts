...
import { HAS_ADMIN_CREDS, getAdminDb } from '@/app/lib/firebaseAdmin';
...
/**
 * Resuelve una sola vez qué DB usar:
 * - Admin (node) si existen credenciales de Service Account
 * - Web SDK (cliente) en caso contrario
 */
const getDbMode = cache(async (): Promise<DbMode> => {
  ...
  // Fallback Web SDK (usa tus NEXT_PUBLIC_*)
  try {
    const { FbDB } = await import('@/app/lib/services/firebase');
    return { admin: false, db: FbDB };
  } catch (e) {
    // Si por algún motivo falla el import del Web SDK, no rompas el render
    console.warn('[meta.server] No se pudo cargar Web SDK, devolviendo DB nula:', e);
    return { admin: false, db: null };
  }
});

/** Construye una referencia tipo Admin: collection/doc/collection/doc/... */
...
/**
 * Lee un documento arbitrario (alternando coll/doc en `segments`) devolviendo {} si no existe.
 * Usa Admin si hay SA; de lo contrario usa Web SDK.
 */
