...
/* ─ Notas:
   • Este archivo no usa claims para subir rol.
   • El rol efectivo lo dicta el server (/api/acl/role), que prioriza hard-code.
   • Si la API falla, el rol queda 'user' (seguro).
*/


/* ─────────────────────────────────────────────────────────
DOC: Auth Context — complements/components/AuthenticationComp/AuthContext.tsx
QUÉ HACE:
  Ofrece un React Context para autenticación (usuario actual, loading, error) y acciones (signIn/out).
  Se integra con Firebase Auth y sincroniza UserState del proyecto.

API / EXPORTS / RUTA:
  — export interface AuthState { user: UserState|null; loading: boolean; error?: string|null }
  — export interface AuthActions {
      signInEmailPassword(email:string, pass:string): Promise<void>
      signInProvider(p:"google"|"facebook"|"apple"): Promise<void>
      signOut(): Promise<void>
    }
  — export const AuthContext = React.createContext<{state:AuthState; actions:AuthActions}>(/*...* /)
  — export function AuthProvider({ children }: { children:React.ReactNode }): JSX.Element
  — export function useAuth(): { state:AuthState; actions:AuthActions }

USO (ejemplo completo):
  import { AuthProvider, useAuth } from "@/complements/components/AuthenticationComp/AuthContext";
  <AuthProvider><App/></AuthProvider>
  // En componente:
  const { state:{user,loading}, actions:{signOut} } = useAuth();

NOTAS CLAVE:
  — Solo cliente (usa onAuthStateChanged). Evitar uso en Server Components.
  — Limpiar listeners en unmount. Manejar persistencia (local/session) fuera o vía opciones.
  — No guardar tokens en storage. Reglas de Firestore deben aplicar por claims/roles.

DEPENDENCIAS:
  firebase/auth · "@/app/lib/services/firebase" · "@/context/UserState"
────────────────────────────────────────────────────────── */
/* ─────────────────────────────────────────────────────────
DOC: USO (ejemplo completo) — complements/components/AuthenticationComp/AuthContext.tsx
  "use client";
  import { AuthProvider, useAuth } from "@/complements/components/AuthenticationComp/AuthContext";
  ...
  // Props/valores:
  // AuthProvider: sin props obligatorios.
  // useAuth(): devuelve { state: { user?:UserState|null, loading:boolean, error?:string|null }, actions:{ signInEmailPassword, signInProvider, signOut } }
────────────────────────────────────────────────────────── */
